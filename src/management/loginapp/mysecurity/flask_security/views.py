...
def _render_json(form, include_user=True, include_auth_token=False):
    ...
    return jsonify(dict(meta=dict(code=code), response=response)), 200
...
def logout():
    ...
    if request.json:
        return jsonify({"meta":{"code":200},"response":{}})
    else:
...
def confirm_email(token):
    ...
    if not user or invalid:
        ...
        return jsonify({"meta":{"code":400},"response":{"errors":{"token":["token invalid"]}}})
    if expired:
...
def reset_password(token):
    ...
    if invalid:
        ...
        return '{"meta":{"code":400},"response":{"errors":{"token":["token invalid"]}}}'
    if expired:
        send_reset_password_instructions(user)
        do_flash(*get_message('PASSWORD_RESET_EXPIRED', email=user.email,
                              within=_security.reset_password_within))
        return '{"meta":{"code":401},"response":{"errors":{"token":["token expired"]}}}'
