...
from myflask_login import current_user
...
#内部审计平台不校验email
email_validator = Required(message='INVALID_EMAIL_ADDRESS')
password_required = Required(message='PASSWORD_NOT_PROVIDED')
password_length = Length(min=1, max=128, message='PASSWORD_INVALID_LENGTH')
...
class PhoneFormMixin():
    phoneno = StringField(
        get_form_field_label('phoneno'),
        validators=[email_required])

class CompanyFormMixin():
    company = StringField(
        get_form_field_label('company'),
        validators=[email_required])
...
class ConfirmRegisterForm(Form, RegisterFormMixin, PhoneFormMixin, UNameFormMixin,
                          CompanyFormMixin, UniqueEmailFormMixin, NewPasswordFormMixin):
    pass
